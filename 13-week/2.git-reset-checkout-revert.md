...
```bash
$ git reset --hard 7c26162 # 나누기 함수 구현
...
git reset 명령을 이용해 다시 git cherry-pick max를 한 시점으로 되돌아간다.

```bash
$ git log --oneline

$ git reset --hard dde3fe8 # git cherry-pick max 을 한 시점으로 원복

```

```bash
$ git revert 1baa34d # 최소값 함수 구현(Math.min)
$ git revert dde3fe8 # 최대값 함수 구현(Math.max)
